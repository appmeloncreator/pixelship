(()=>{
  ...
  renderMine=function(){
    ...
    $('#myGrid').innerHTML=mine.length
      ?mine.slice(0,8).map(post=>'<div class="mini" data-id="'+post.id+'"><img src="'+post.image+'" alt="'+esc(post.prompt)+'"><span class="mini-status '+(post.isPublic?'public':'')+'">'+(post.isPublic?'PUBLIC':'PRIVATE')+'</span><div class="mini-tools"><button data-action="edit">EDIT</button><button data-action="'+(post.isPublic?'private':'publish')+'">'+(post.isPublic?'MAKE PRIVATE':'PUBLISH')+'</button><button class="danger" data-action="delete">DELETE</button></div></div>').join('')+'<div class="mini empty">＋ NEW IDEA</div>'
