...
DB_URL=$(sed -n 's/^SUPABASE_DB_URL=//p' "$CONFIG" | tail -n 1 | tr -d '')
...
for file in /app/supabase/schema.sql /app/supabase/002_private_by_default.sql /app/supabase/003_conversations.sql /app/supabase/004_content.sql; do
  name=$(basename "$file")
